...
import { holdForFiling, pendingRequestCount, revealFilingQueue, writeAttachment } from "../claudebridge";
import { courseNames } from "../coursework";
import { t } from "../i18n";
import { FilePickerModal } from "../pickers";
import { type DashboardCard } from "../types";
import { makeClickable } from "../ui";
import { findCourseInText, HOLDING_FOLDER } from "../vaultfiling";
import { type HomeView } from "../view";
import { type CardDefinition, type CardEditorContext } from "./definition";

/**
 * Add detail — attach a page, files or prose to a note, and let Claude file it.
 *
 ...
 *
 * None of those are decided here. Everything the user supplies is written into
 * the vault immediately — prose into a holding note, files into the holding
 * folder — and a request goes to `Claude/inbox/` naming the target note and
 * pointing at the vault's rules. See `claudebridge.ts` for why the decision
 * doesn't live in this plugin.
 *
...
/** Where dropped files wait. A subfolder of the holding area so an attachment
 * never sits loose beside the notes that describe it. */
const HOLDING_ATTACHMENTS = `${HOLDING_FOLDER}/attachments`;
...
export function renderDetail(view: HomeView, card: DashboardCard, body: HTMLElement): void {
	...
	switch (card.size) {
		case "small": {
			...
			tile.createDiv({ cls: "sbd-detail-title", text: t().cards.detail.addDetail });
			tile.createDiv({ cls: "sbd-detail-sub", text: t().cards.detail.pageFilesNotes });
			clickable(tile, open, t().cards.detail.addDetail);
			break;
		}
		case "medium": {
			const head = body.createDiv("sbd-card-headline");
			setIcon(head.createDiv("sbd-card-headline-icon"), "paperclip");
			head.createDiv({ cls: "sbd-card-headline-text", text: t().cards.detail.addDetail });
			const row = body.createDiv("sbd-detail-row");
			searchField(row, open);
			chip(row, t().cards.detail.dropFiles, open);
			chip(row, t().cards.detail.writeNotes, open);
			break;
		}
		case "large":
		case "xlarge": {
			...
			text.createDiv({ cls: "sbd-card-eyebrow", text: t().cards.detail.eyebrow });
			text.createDiv({
				cls: "sbd-course-name is-large",
				text: card.size === "xlarge" ? t().cards.detail.headlineLong : t().cards.detail.addDetail,
			});
			const wrap = body.createDiv(card.size === "xlarge" ? "sbd-detail-cols" : "sbd-detail-stack");
			action(wrap, "link", t().cards.detail.linkPage, t().cards.detail.linkPageSub, open, "glass");
			action(wrap, "download", t().cards.detail.dropFiles, t().cards.detail.dropFilesSub, open, "sheet");
			action(wrap, "pencil", t().cards.detail.writeNotes, t().cards.detail.writeNotesSub, open, "sheet");
			break;
		}
	}

	// A queue nothing is draining is the one failure mode worth surfacing on the
	// card itself: the plugin has done its half, and the items are still waiting.
	if (pending > 0 && card.size !== "small") {
		const queue = body.createDiv({ cls: "sbd-detail-queue", text: t().cards.detail.pending(pending) });
		clickable(queue, () => void revealFilingQueue(view.app), t().cards.detail.pending(pending));
	}
}
 ...
 *
 * Save writes all three into the vault and queues the filing decision; it never
 * edits the target note directly. Editing it would mean choosing a heading and
 * a wording, which is the judgement being deferred — and a note silently
 * appended to in the wrong place is harder to notice than an item sitting in an
 * inbox.
 */
class DetailModal extends Modal {
	...
	/**
	 * Write everything down and queue the decisions.
	 *
	 * Files are written before the request that mentions them, so the request
	 * can never point at an attachment that failed to land.
	 */
	private async save(): Promise<void> {
		...
		for (const file of this.files) {
			...
			try {
				written = await writeAttachment(app, HOLDING_ATTACHMENTS, file.name, await file.arrayBuffer());
			} catch {
				written = null;
			}
			if (!written) {
			...
			await holdForFiling(
				app,
				{
					...
					details: {
						...
						[t().cards.detail.detailAttachment]: written.path,
...
/** Attach a page, files or notes to a note, and hand the filing to Claude. */
export const detailCard: CardDefinition<"detail"> = {
	...
	// The pending-requests line is read from the vault, so the card follows it.
	liveness: { mode: "vault" },
};
